...
### 1.2 Hộp thoại phóng to ảnh (Lightbox Viewer)
- **Mục tiêu:** Cho phép người dùng bấm vào ảnh trong Markdown Preview (Mô tả/Chi tiết) hoặc danh sách File đính kèm để xem phóng to ngay trên màn hình.
- **Nguyên lý:**
  ...
  - Điều chỉnh danh sách file đính kèm: định dạng ảnh (`image/*`) sẽ hiển thị nút `<button>` mở Lightbox thay cho link `<a>` mở tab mới.
  - Hiển thị lớp phủ mờ tối `z-[60]` có nút Close và đóng khi bấm ra ngoài ảnh.
...
- **Kiểm thử Lightbox ảnh Markdown:** Mở Modal/Popover, click vào ảnh trong phần Xem trước mô tả/chi tiết, kiểm tra xem ảnh phóng to lên có lớp phủ mờ tối và nút đóng.
- **Kiểm thử Lightbox ảnh Đính kèm:** Thêm tệp ảnh đính kèm, click vào tên ảnh, xác minh ảnh được phóng to qua Lightbox thay vì mở tab mới.
- **Kiểm thử thoát Lightbox:** Click ra ngoài ảnh hoặc click nút Đóng, kiểm tra xem Lightbox biến mất hoàn toàn.
